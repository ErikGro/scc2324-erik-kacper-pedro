...
public class AzureManagement {
	...
	static final String MY_SUFFIX = "68806"; // Add your v2 here
	...
	static final String[] BLOB_CONTAINERS = { "images-house", "images-user" };	// TODO: Containers to add to the blob storage

	static final Region[] REGIONS = new Region[] { Region.EUROPE_WEST}; // TODO: Regions to use
	...
	static final String[] AZURE_APP_NAME = Arrays.stream(REGIONS).map(reg -> "scc-backend" + reg.name() + MY_SUFFIX)
			.toArray(String[]::new);
	...
	static final String[] AZURE_REDIS_NAME = Arrays.stream(REGIONS).map(reg -> "sccredis" + reg.name() + MY_SUFFIX)
			.toArray(String[]::new);
		
	// Name of Azure functions to be launched in each regions TODO: Azure function
	static final String[] AZURE_FUNCTIONS_NAME = Arrays.stream(REGIONS).map(reg -> "scc-fun" + reg.name() + MY_SUFFIX)
			.toArray(String[]::new);
	...
	public synchronized static void dumpStorageKey(Map<String, String> props, String propFilename,
		...
		synchronized (props) {
			props.put("DB_BLOB_CONNECTION_STRING", key);

		}

		synchronized (AzureManagement.class) {
			Files.write(Paths.get(propFilename), ("DB_BLOB_CONNECTION_STRING=" + key + "\n").getBytes(),
		...
		if (functionName != null) {
			...
			cmd.append(rgName);
			cmd.append(" --settings \"DB_BLOB_CONNECTION_STRING=");
			cmd.append(key);
			cmd.append("\"\n");
		}
		if (appName != null) {
	...
	public synchronized static void dumpCosmosDBKey(Map<String, String> props, String propFilename,
		...
		synchronized (AzureManagement.class) {
			Files.write(Paths.get(propFilename),
					("DB_KEY=" + account.listKeys().primaryMasterKey() + "\n").getBytes(),
					StandardOpenOption.APPEND);
			Files.write(Paths.get(propFilename), ("DB_CONNECTION_URL=" + account.documentEndpoint() + "\n").getBytes(),
					StandardOpenOption.APPEND);
			Files.write(Paths.get(propFilename), ("DB_NAME=" + databaseName + "\n").getBytes(),
					StandardOpenOption.APPEND);
		}
		synchronized (props) {
			props.put("DB_KEY", account.listKeys().primaryMasterKey());
			props.put("DB_CONNECTION_URL", account.documentEndpoint());
			props.put("DB_NAME", databaseName);
		}
		...
		if (appName != null) {
			...
			cmd.append(" --resource-group ");
			cmd.append(rgName);
			cmd.append(" --settings \"DB_CONNECTION_URL=");
	...
	public synchronized static void dumpRedisCacheInfo(Map<String, String> props, String propFilename, 
		...
		synchronized (AzureManagement.class) {
			Files.write(Paths.get(propFilename), ("REDIS_KEY=" + redisAccessKey.primaryKey() + "\n").getBytes(),
					StandardOpenOption.APPEND);
			Files.write(Paths.get(propFilename), ("REDIS_HOSTNAME=" + cache.hostname() + "\n").getBytes(),
					StandardOpenOption.APPEND);
		}
		synchronized (props) {
			props.put("REDIS_KEY", redisAccessKey.primaryKey());
			props.put("REDIS_HOSTNAME", cache.hostname());
		}
		StringBuffer cmd = new StringBuffer();
		if (appName != null) {
			cmd.append("az functionapp config appsettings set --name ");
			...
			cmd.append(" --resource-group ");
			cmd.append(rgName);
			cmd.append(" --settings \"REDIS_HOSTNAME=");
					...
					Thread th = new Thread(() -> {
						try {
							...
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "users", "/id",
									null);
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "rental", "/id",
									null);
